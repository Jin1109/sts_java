...
/*
 * @ControllerAdvice annotation을 통해서 이 클래스의 객체가 컨트롤러에서 발생하는
   Exception을 전문적으로 처리하는 클래스라는 것을 명시합니다.
   
   만드는 방식은 다음과 같습니다.
   1. 클래스에 @ControllerAdvice라는 annotation 처리
   2. 각 메서드에 @ExceptionHandler를 이용해서 적절한 타입의 Exception을 처리
 */
@ControllerAdvice
public class ExceptionController {
	...
	/* common메서드는 Exception 타입으로 처리하는 모든 예외를 처리하도록 설정 */
	@ExceptionHandler
